...
app.post('/new_user', async (req, res) => {
    ...
    try {
        ...
        if(!user.rows[0]){
            user = await pool.query('INSERT INTO users(email, password, watchedMovies) VALUES ($1, $2, $3) RETURNING *', [email, password, movies]);
        }
...
app.put('/users/:userId/movies', async (req, res) => {
    ...
    try{
        ...
        await pool.query('UPDATE users SET watchedMovies = $1 WHERE user_id = $2', [user.watchedmovies, userId]);
